...
const ImageViewer = (() => {
  ...
  const open = (src) => {
    ...
    document.getElementById('img-viewer-zoom').textContent = '100%';
    ...
    window.onmouseup = () => { _dragging = false; img.style.cursor = 'grab'; };
    ...
    img.ontouchmove = (e) => {
      if (e.touches.length === 2) {
        ...
        _scale = Math.max(0.25, Math.min(5, _scale * (dist / _lastTouchDist)));
        _lastTouchDist = dist;
        _applyTransform(img);
      } else if (_dragging) {
        _x += e.touches[0].clientX - _lastX;
        _y += e.touches[0].clientY - _lastY;
        _lastX = e.touches[0].clientX;
        _lastY = e.touches[0].clientY;
        _applyTransform(img);
      }
    ...
    img.ontouchend = () => { _dragging = false; };

    overlay.onwheel = (e) => {
      e.preventDefault();
      _scale = Math.max(0.25, Math.min(5, _scale + (e.deltaY < 0 ? 0.15 : -0.15)));
      _applyTransform(img);
    };
  };

  const _applyTransform = (img) => {
    img.style.transform = `translate(${_x}px, ${_y}px) scale(${_scale})`;
    const pct = Math.round(_scale * 100);
    const lbl = document.getElementById('img-viewer-zoom');
    if (lbl) lbl.textContent = pct + '%';
  };

  const zoom = (delta) => {
    const img = document.getElementById('img-viewer-img');
    _scale = Math.max(0.25, Math.min(5, _scale + delta));
    _applyTransform(img);
  };
  ...
  return { open, zoom, reset, close };
})();

const wikiApplyColorWS = (color) => {
  const el = document.getElementById('ws-editor-free');
  if (!el) return;
  el.focus();
...
const wikiApplyHighlightWS = (color) => {
  const el = document.getElementById('ws-editor-free');
  if (!el) return;
  el.focus();
...
const InlineImgDrag = (() => {
  ...
  const start = (e, img) => {
    ...
    _imgLeft = parseInt(img.style.left) || 0;
    _imgTop  = parseInt(img.style.top)  || 0;
    img.style.cursor = 'grabbing';
    ...
    const onMove = (ev) => {
      ...
      img.style.left = (_imgLeft + dx) + 'px';
      img.style.top  = (_imgTop  + dy) + 'px';
    };
    const onUp = () => {
      _dragging = false;
